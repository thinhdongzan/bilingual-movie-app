function MovieCard({ movie }) {
	return (
		<div className="group bg-neutral-900/40 rounded-xl overflow-hidden shadow-md border border-neutral-800 transition-transform duration-200 hover:-translate-y-1 hover:shadow-2xl hover:border-neutral-700 cursor-pointer">
			<div className="relative aspect-[3/5] w-full bg-neutral-800 overflow-hidden">
			...
			<div className="px-4 py-3">
				<p className="truncate text-base font-medium text-white">{movie.title}</p>
			</div>
			<div className="px-4 pb-4">
				<p className="text-sm text-white/60">⭐ {movie.voteAverage.toFixed(1)} | {movie.releaseDate.split('-')[0]}</p>
			</div>
		</div>
	);
}
